...
`scripts/checks/*.check`を追加・変更するときは、先に配布版の[fast-check registry contract](https://github.com/masuidrive/pdh/blob/XXXXXXX/codex/templates/checks/README.md)を読む。導入先では同じ仕様が`scripts/checks/README.md`にある。field、glob、exclude、allow、placeholder、安全なcommand実行の詳細はそのREADMEに従い、このskillへ複製しない。

## 型を選ぶ

- `pattern`: 禁止する文字列invariantをPOSIX EREで検出する。一般的なstyle検査を再実装しない。
- `max_lines`: file全体の行数上限を守る。既存の大きなfileを例外にする場合だけexact-path `allow`を使う。
- `linter_command`: projectが所有するlinterを対象fileへ流す。
- `required_paths`: 消えては困るfileをexact pathで名指しする。
...
3. fileの中身を守るcheckなら、そのfileが消える経路（上流のdirectory上書き、merge事故、伝播しなかったrename）を確認する。経路があるなら`required_paths` checkを併置する。
...
Based on https://github.com/masuidrive/pdh/blob/XXXXXXX/codex/skills/pdh-check-writing/SKILL.md
