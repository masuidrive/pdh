...
**図は SVG を直接書く。**`viewBox` を指定し、`width="100%"` `height="auto"` を付け、色は `var(--accent)` `var(--ok)` `var(--warn)` `var(--muted)` `var(--line)` で指定する（**固定色は暗い配色で読めなくなる**）。⚠ **mermaid は寸法が中身で決まるため、横長に描きたくても制御できない。**横長の図は入れ物に `class="media wide"` を付ける — **入れ物だけが広がり、svg の寸法は書き手のものになる。**見本は `kit/ui-sample.html`。
