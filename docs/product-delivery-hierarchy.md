...
- Epic クローズ時に Retrospective セクションに振り返りを記録することを推奨する（プロセスの改善点・再利用可能なパターン・発見した知見）。
...
- 識別済みリスクが顕在化した場合は、影響範囲を評価し、対応策を実行する。影響が大きい場合（スコープ変更・技術方針の転換が必要）はユーザに相談する。
...
### Risks
<!-- この Epic で想定されるリスクと対応策。
     技術的リスク（未経験の技術、パフォーマンス懸念）、
     外部依存リスク（API 変更、サービス停止）、
     スコープリスク（要件の曖昧さ）など。
     各リスクに対して回避・軽減・受容のいずれかを選ぶ。
     なければ省略。 -->
...
### Retrospective
<!-- Epic 完了後の振り返り。D3 クローズ時に記録する。
     - うまくいったこと（再利用したいパターン・判断）
     - うまくいかなかったこと（次回改善したいプロセス・判断）
     - 発見した知見（技術的な学び、ドメイン知識）
     なければ省略。 -->
...
### Risks
<!-- この Ticket の実装で想定されるリスクと対応策。
     技術的リスク（未検証の API、複雑なロジック、パフォーマンス影響）や
     外部依存リスク（バージョン互換性、API 制限）など。
     各リスクに対して回避・軽減・受容のいずれかを選ぶ。
     なければ省略。 -->
