...
- **Ticket の Acceptance Criteria** — **観察可能な振る舞い** で書く。「〜できる」「〜が返る」「〜が表示される」など。曖昧な品質形容詞（「適切に」「正しく」）は避ける。プロセス要件（「レビュー済み」「テストパス」）は AC に書かない。ワークフロー（SKILL.md）と作業ノート（note）が保証する。
  ...
  **判定は 1 問で行う — 「承認者がこれを自分で確かめるとしたら、何をするか」。**答えが書けなければ、その AC は承認者の言葉になっていない。実装の言葉で決めた中身は `確定判断 (Design Decisions)` へ移し、AC には場面を残す。約束の中身は減らさず、読み手だけを変える。⚠ **内部の識別子は禁止語ではない** — 悪いのはそれが約束の本体になっていることで、「管理者が設定を保存できる（実装上は `PATCH /api/…`）」は読める。⚠ **平易な言葉への言い換えでもない** — 読み手は上級エンジニアであり、避けるのは技術的な内容ではなく ticket の外を見に行かせること。⚠ **非退行の AC は「できるようになる」の型に収まらない** — そこは «登場人物» だけを必須にする。定義と書き直しの例は `pdh-dev` skill にある。
...
<!-- 完了を判定できる条件。プロダクトの観察可能な振る舞いだけを書く。
     ...
     判定は 1 問 —「承認者がこれを自分で確かめるとしたら、何をするか」。
     書けなければ、それは承認者の言葉になっていない。実装の言葉で決めた中身は
     確定判断 (Design Decisions) へ移す。
