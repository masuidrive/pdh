...
- `interrupt <run-id>` は current step に clarification artifact を作成して run を `interrupted` にし、`answer <run-id>` が解決するまで provider 実行を止める。解決済みの割り込みは次の provider prompt に含める。
