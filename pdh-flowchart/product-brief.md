...
システムは `current-note.md` と `current-ticket.md` を読み書きできるか、最低限更新提案を生成しなければならない。
...
1. flow 定義は YAML と TypeScript のどちらを正本にするか
2. note/ticket 更新は自動書込か patch 提案か
3. review step で複数 reviewer を MVP に含めるか
4. human gate は CLI のみで十分か、簡易 Web UI を先に入れるか
5. repo ごとの差分設定をどう吸収するか
