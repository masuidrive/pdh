...
- **skills**: `skills/` に Claude Code skill の実体、`templates/.agents/skills/` に Codex 用 wrapper
- **導入・更新経路**: README を coding agent に読ませて導入。更新は `pdh-update` skill

主要フロー:
1. 開発者が自プロジェクトで agent に「PDH の README を読んで導入して」と指示 → ticket.sh 導入 + ファイル配置 + Product Brief 雛形作成まで完了する
...
- `AI-2` skill の実体は `skills/`（配布先 `.claude/skills/`）に置く。`templates/.agents/skills/` は実体を読む wrapper のみとし、ワークフローを複製しない
- `AI-3` 配布テンプレートには、テンプレート自身の使い方説明を書かない。導入手順は README、運用ルールは `docs/product-delivery-hierarchy.md` が正
...
- 新規プロジェクトで README を agent に読ませるだけで導入が完了する
...
- **自動検査は持つ**（2026-07-18）。`scripts/test-all.sh` = fast-checks + check-distribution + shell 構文。配布物間の**重複検出**は grep で表現できないため自動化を見送り、`AI-1` / `AI-3` として brief と `CLAUDE.md` のルールで担保する
