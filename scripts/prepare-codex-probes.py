...
def main():
    ...
    parser.add_argument('--source-set', choices=('codex', 'legacy'), default='codex',
                        help='legacy reads the pre-split root skills at an older revision')
    ...
    for source in SOURCES:
        if args.source_set == 'codex' and source.startswith('skills/'):
            source = 'codex/' + source
    ...
    (args.output/'manifest.json').write_text(json.dumps({
        'revision': args.revision, 'source_set': args.source_set, 'kind': args.kind, 'sources': manifest,
