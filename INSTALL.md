...
| `tmp/pdh/skills/pdh-decision-board-base/` | `.claude/skills/pdh-decision-board-base/` | 判断ボードの共通規則と renderer（分冊 `*.md` / `evals/` / `kit/`（CSS・JS・見本）/ `tools/`（組み立て・静的検査・読む量の測定。bash と awk だけで動く）を**ディレクトリごと**コピーする） |
