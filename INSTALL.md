...
- **新規に導入する**: [方法 1](#方法-1-claude-code-に設定させる)（agent に任せる）または [方法 2](#方法-2-手動でセットアップ) の §0〜§2.5〜§4〜§9
- **導入済みを更新する**: [§3 既存ファイルのアップデート](#3-既存ファイルのアップデート) と [§3.1 既知の移行手順](#31-既知の移行手順)
...
for s in pdh-dev pdh-coding pdh-update tmux-director; do
...
4. **新規ファイルの付随設定 (重要)**: 新規ファイルには単独配置だけでは機能しないものがある。下表に該当する追加があれば、対応する章を見て `.claude/settings.json` / `vitest.config.ts` / `scripts/test-all.sh` 等を更新する:
   ...
   |---|---|---|
   | `scripts/hookbus.js` | §2.5.b + §6.3 + README「tmux Director」 | `.claude/settings.json` に 5 hook ブロック、`vitest.config.ts` に scripts project (includeSource)、`package.json` に `test:scripts`、`scripts/test-all.sh` に `run "test:scripts"` 行 |
   | 新規 `skills/<名前>/SKILL.md` (将来追加されたもの) | 追加された skill の README / ヘッダコメント | skill 固有の settings.json / env / script があれば個別対応 |
   | 新規 `templates/` | §2 / §その他 | テンプレートによる。本ガイドを再読し該当章を確認 |
...
5. **既存ファイルの差分マージ**: `Based on` 行があるファイルごとに差分を取得・反映する:
   ...
   - **本ガイドが指定する設定 (settings.json / vitest.config.ts / scripts/test-all.sh)**: プロジェクト固有のカスタマイズが入っているので上書きしない。代わりに 本ガイド §2.5 / §6 / §6.3 と README「tmux Director」で推奨される項目が含まれているかをレビューし、抜けていたら追加する
6. **削除されたファイルの撤去**: 旧 commit から HEAD で `D` (削除) になったファイルがあれば、プロジェクト側から除去する。該当する設定 (settings.json / vitest.config.ts / scripts/test-all.sh) も必要なら撤去。上の表の逆手順。**過去の major refactor で `epics/` ディレクトリ / `epic-creator` skill / Light Full flow / PD-A / PD-B / PD-D / PD-C-2/3/4/5/8 phase は廃止された**。旧バージョンから upgrade する場合は project 側からこれらの参照を撤去する
   ...
   **⚠ 削除が「置き換え」である場合がある。** 下の §3.1 に該当する移行があれば、単に消すのではなくそちらの手順に従う。

6.5. **既知の移行手順 (§3.1)**: 下の §3.1 を読み、自分のプロジェクトが該当する項目をすべて適用する
