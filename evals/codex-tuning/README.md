...
```bash
python3 scripts/prepare-codex-probes.py --revision 37f1763 --source-set legacy \
...
配布セット分割後は `codex/skills/` と共有 `docs/` を読む。旧構造のrevisionを再測定する場合は `--source-set legacy` を明示する。移植時の検証と保留範囲は [codex-set.md](codex-set.md) に記録する。
