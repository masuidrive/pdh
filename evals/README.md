...
実案件（llmhub）の Slack・ticket の版・transcript・判断ボードをそのまま持つデータセットは `evals/private/`（git 管理外、この機械でだけ再生成できる）にあり、切り出し方は同 `TEST-DESIGN.md` にある。合成 fixture で仕込めない «後から分かった事実» を正解にするときはそちらを使う。
