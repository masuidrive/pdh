...
**`tools/check-static.sh` は、これを理由に「節・カード・AC・判断・見出しの数や有無は検査に入れない（板の形は案件ごとに違うため）」と決めている。**同じ形の判断は `PDH-AGENTS.md`「worker の PASS は入力であって承認ではない」にもある。**新しい検査を思いつく前に、その対象を «測らない» と決めた記述が既にあることが多い。**
