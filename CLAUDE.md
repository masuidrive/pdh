...
### ⚠ 自由文を、行数や件数で決定論的に評価しない

規則・ticket・判断ボードのような**自由文に対して「AC が全件あるか」「指摘を全件出したか」を機械で測ることはできない。**測るには書式を固定するしかなく、固定していない書式を数えた検査は、**当たる形の成果物にだけ当たり、外れた形では沈黙して通る。**

**落ちない検査は無いのと同じだが、沈黙して間違える検査はそれより悪い** — 通ったことが担保に見える。

`tools/check-static.sh` が「節・カード・AC・判断・見出しの数や有無は検査に入れない（板の形は案件ごとに違うため）」と決めているのは、この理由による。

機械で測れるのは**書式が定まっているもの**だけである。
...
⚠ **測れないものを «測れる» と書かない。**担保できていないなら、担保できていないと書く。**実際より強く見える記述は、その検査を通した人に «確かめた» と誤認させる。**内容が要件を満たしているかを見たいなら、`evals/` のように人か別 agent が読む形にする（決定論ではないので `test-all.sh` には入らない）。
