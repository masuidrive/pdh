...
**選択の見た目、選択中の 1 枚だけが塗られること、localStorage への保存と復元、コピーの 3 段、送信ボタンの出し分け、目次の ✓、進捗バッジの移動、明暗テーマ、行頭禁則 — どれも kit が保証する。板を書く人は class と `data-*` を書くだけで、そうなっているかを確かめない。**

**面の自動縮小は kit の «最後の逃げ道» であって（`deck.js` の `MIN`）、材料が読める大きさで収まっているか＝縮小に頼っていないかは板の作者が描画して確かめる**（reader-profile 自己検査 7）。⚠ **`data-fit` は描画後にしか読めず、`tools/check-static.sh` / `build.sh` では静的に測れない** — 収まりは組んだ直後の静的検査ではなく描画で見る。
...
CSS は `kit/board.css` にある。**色・枠を style 属性で足さない** — 意味は class が持つ。`ol.steps`（順番が意味を持つ列挙。順序が無い列挙には使わない）、`ul.tl`（時系列。1 行 = 事実 1 つ、日付必須）、`blockquote.quote`（報告・発言は要約で潰さず原文で引用。出典必須）、`figure.fig`（画像・SVG・mermaid の枠。caption に「何を見てほしいか」を必ず書く）、`details.fold`（根拠の詳細を 1 手で開く畳み。**summary は結論そのものを書く**）、目次（**`page.js` が節から自動で作る。書き手は書かない** — `<section id>` に `<h2>` を入れておけば、節が 5 つを超えたとき目次が現れる）。
...
**判定は語ではなく `.tag` で置く。**`.tag.ok`（通った）/ `.partial`（部分的）/ `.ng`（通らない）/ `.na`（対応なし）の 4 つは**判定の軸**で、`.rec` / `.alt` / `.care`（推奨の軸）と混ぜない。**受け入れ条件の一覧と、毎回の確認は 1 行 1 判定なので、この 2 か所には必ず付ける。**通っていない行だけ地が薄く塗られる（kit が `tr:has(.tag.ng)` で行う。**全行を塗ると差が消えるので、書く側で塗らない**）。

**図は用途で選ぶ。**

- **節点の配置と線の引き回しを計算させたい図は mermaid**（`pre class="mermaid"`）— sequence・分岐の多い flowchart・state・ER・class。**ライフラインの位置や矢印の高さを手で計算させない。**
- **形と縦横比を自分で決めたい図は SVG を直接書く** — 段の流れ、並べて比べる図、画面の模式、数の大小。`viewBox` を指定し、`width="100%"` `height="auto"` を付け、色は `var(--accent)` `var(--ok)` `var(--warn)` `var(--muted)` `var(--line)` で指定する（**固定色は暗い配色で読めなくなる**）。
- ⚠ **mermaid は寸法が中身で決まるので、横長に描きたくても制御できない。**それが要るときだけ SVG にする。⚠ **逆に、分岐の多い図を手書き SVG でやらせると崩れる。**

横長の図は入れ物に `class="media wide"` を付ける — **入れ物だけが広がり、svg の寸法は書き手のものになる。**⚠ **svg に `width:100%` を当てて引き伸ばさない**（小さな図で字まで比例拡大されて読めなくなる。2026-09-18 に実測）。見本は `kit/ui-sample.html`。
...
| `.tag` | `` `達した` `` のように語のまま |
| `.card.answer-choice` の選択肢 | `- [ ]` |
| `figure.fig` の `img` | `![caption](画像の src)` |
| `dl` / `dt` / `dd` | `**用語**` の行 + 説明の行（行末の `\` で改行を固定）。⚠ **`dl` に当たる CSS は kit に無い**ので、板では素の見た目になる |
| `strong` / `em` | `**…**` / `*…*`。⚠ **句読点・括弧・空白は強調の外へ出す**（`**出ません**。触った`）— 中に入れると GitHub が記号ごと素で出す（2026-09-18 に実測） |
| `.answer-jump`（回答欄へのリンク） | ⚠ **落ちる**（Markdown 側に回答欄が無い） |
| ⚠ **`svg`** | ⚠ **落ちる。**「図は HTML の board にあります」に置き換わるので、**`--url` を必ず添える**（URL は出力の **先頭** に出る） |
