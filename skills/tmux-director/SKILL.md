...
## MAX_ITERATIONS の設定
タイムアウト（最大回数到達）すると Monitor → Director → 新 Monitor の再起動に空白期間が生じ、その間の AskUserQuestion を見逃す。**常に 240（15秒 × 240 = 約1時間）を設定する。** 作業内容による調整は不要。
...
| 報告の種類 | Director の行動 |
|---|---|
| **入力待ち** | **まず TD-3.5 フェーズ遷移検証を実施**。検証 OK → ユーザに報告し判断を仰ぐ。検証 NG → window に是正指示を送信し Monitor 再起動。次ステップあり → 指示を送信し Monitor 再起動。**重要: Window への指示は常に 1 フェーズ分のみ。「PD-C-4 をやって、その後実装も進めて」のように複数フェーズをまとめて指示しない。ユーザ確認ポイント（PD-C-5 実装承認、PD-C-10 クローズ）を飛ばす原因になる** |
...
**重要: チケットの規模に関わらず、この検証を省略してはならない。** 小規模チケットでも必ず `current-note.md` と `current-ticket.md` を読み、レビュアーテーブル・AC 達成状況・テスト結果を確認してからユーザに報告する。Monitor の報告だけで判断せず、自分の目で note を確認すること。
...
2. **`current-note.md`** の該当セクション（例: 「PD-C-4. 計画レビュー結果」「PD-C-7. 品質検証結果」）を Read し、完了条件が満たされているか確認する
3. **`current-ticket.md`** の AC を Read し、AC に関わる検証が行われているか確認する（特に PD-C-8/PD-C-9）
...
| **既存問題・残課題の扱い** | `current-note.md` に「対応検討」「スコープ外」「別チケット」「future-list」等と記載された項目がないか確認する。ある場合は **クローズ前にユーザに個別に提示し、対応方針（新チケット / future-list / 対応不要 / 今回のチケットで対応）の判断を仰ぐ** |
...
- **window が PDH ワークフロー（PD-C-1〜PD-C-10）に従っているか、Monitor の報告で確認する**
...
| タイミング | 報告内容 |
|---|---|
| **計画完了後・実装開始前（PD-C-4 → PD-C-5 実装承認）** | 計画内容（設計判断、ファイル変更計画、E2E テスト手順、懸念事項） |
| **コードレビュー後・チケットクローズ前（PD-C-8 → PD-C-10 クローズ）** | レビュー結果（テスト結果、AC 達成状況、実環境動作確認結果、残課題）。**特に「既存問題」「対応検討」「スコープ外」と記載された項目は個別に列挙し、それぞれの対応方針をユーザに確認する** |
| **window が選択肢のある判断を行う場面（一般原則）** | 残課題の扱い（チケット化 vs future-list）、設計方針の選択、scope 外事項の対応方針 |

**Gate 到達時のセルフチェック指示**: 上記の gate タイミングで window が次フェーズへの遷移を報告してきた場合、ユーザに報告する **前に** まず window に以下を送信する:

```
次のフェーズに進む前に、pdh-dev ワークフロー（.claude/skills/pdh-dev/SKILL.md）の [PD-C-N] ステップの完了条件を読み直し、current-note.md のログと照合して、全てのステップを正しく踏んだか確認してください。ステップ遷移宣言（[PD-C-X] -> [PD-C-Y] の形式）が抜けていれば補完してください。確認結果を報告してください。
```

window のセルフチェック結果を受け取ってから、ユーザへの報告に進む。これにより window 自身がフロー遵守を再確認でき、Director が検知する前にステップ漏れを自己修正する機会を与える。
...
- フェーズが省略されていないか（PD-C-6/PD-C-7/PD-C-9/PD-C-10 は省略不可。他の省略はユーザ承認が必要）
- レビューフェーズ（PD-C-4/PD-C-7）で修正後の再レビューが実施されているか
...
| **レビューで既存問題が「対応検討」「スコープ外」「別チケット」と記載されている** | **Director がユーザに AskUserQuestion で背景・選択肢を提示し、対応方針の判断を仰ぐ**（window に判断を任せない） |
| **ユーザ確認なしにフェーズ境界（PD-C-5 実装承認、PD-C-10 クローズ承認）を越えて進んでいる** | 即座に window を止め、ユーザに状況報告して承認を得る。ユーザが window に質問・会話しただけでは承認にならない。pdh-dev が定義する「明示的な意思表示（OK/y/yes/進めて）」のみ有効 |
...
- PD-C-6（実装）/ PD-C-7（品質検証）/ PD-C-9（完了検証）/ PD-C-10（クローズ）は省略不可（CLAUDE.md ルール）
- PDH ワークフローから大きく外れる場合は、window への指示を止め、ユーザにその旨を伝えて判断を仰ぐ

---
Based on https://github.com/masuidrive/pdh/blob/8e947f9/skills/tmux-director/SKILL.md
