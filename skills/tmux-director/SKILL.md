...
- **window に「follow-up で済ませる」判断を許さない** — window から「AC 未達だが follow-up ticket を切って close したい」「future-list に回して close したい」等の報告が来た場合、Director はそれを承認せず、**その場で解決する方針**を取る:
  1. 裏取りに必要な環境（API key / credential / 実プロバイダ / 外部サービス等）が欠けている場合は、Director がユーザに必要物を確認し、取得してから window に裏取りを実施させる
  2. 代替裏取り手段（別 provider での等価確認、統合テスト等）を window に整理させ、ユーザ判断を仰ぐ
  3. それでも不可能な場合に限り、deferred としてユーザの明示承認を取る
  deferral はユーザの明示承認事項。window / Director が自発的に future-list 行きを選択してはならない

### 自動監督モードでの事前確認（必須）

ユーザから「自動で判断して進めて」「任せる」「適当に進行させて」等の一任指示を受けた場合でも、**作業着手前に Director が Epic / Ticket の AC / Exit Criteria を読み、以下を洗い出してユーザに確認する**:

- **環境的に裏取りが難しい項目**: 実 API key、実プロバイダ、ハードウェア、特定 OS 拡張、外部サービスが必要な AC / Exit Criterion。例: 「ollama + bge-m3 で日本語意味ヒット」→ ollama と model が必要。「Google Auth で認証」→ OAuth credential が必要
- **解釈に余地がある項目**: 「適切に」「意味ヒットする」「自然な UX」等の定性的な AC。具体的に何を verified の基準とするか確認する
- **複数 Epic / Ticket を跨ぐ項目**: 依存関係がある場合はスコープ境界を確認する

一任指示は「確認を省略してよい」という意味ではない。**未解決の前提を抱えたまま window を走らせると、close 直前に裏取り不能と判明して deferral / follow-up 化の誘惑に負ける**。着手前に前提を潰す。
...
| **AC 未達を「follow-up ticket にすれば close できる」と window が判断する** | Director が止めて、その場で裏取りする方針を取る。必要な環境・credential をユーザに確認し、取得してから裏取り実施。deferral はユーザの明示承認事項 |
| **mock テスト通過だけで `real-env-required` AC を verified とする** | 実プロバイダ / 実環境での確認を指示。実行コマンドと証跡（出力・screenshot）を取得させる |
| **「自動で判断して」と言われたから前提確認をスキップする** | 着手前に AC / Exit Criteria を走査し、環境前提・解釈の余地をユーザに確認する |
