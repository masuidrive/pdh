...
3. **window が選択肢のある判断を行う場面（一般原則）**: 残課題の扱い（チケット化 vs future-list）、設計方針の選択、scope 外事項の対応方針など、複数の選択肢がある判断は window に委ねず、window に選択肢と判断材料を整理させた上でユーザに提示して判断を仰ぐ
...
3. 一意に判断できる（事実確認・手順的な応答） → window に回答を送信 → Monitor を再起動
4. 複数の選択肢がある判断 → **window に判断材料の整理を依頼** → 結果をユーザに提示 → ユーザの判断を window に伝える
5. 判断できない → ユーザに情報と選択肢を提示して指示を仰ぐ
