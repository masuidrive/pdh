...
| **PD-D-3 Epic クローズ** | ゼロベースレビュー後 | Close Summary（達成 Outcome、完了/未完了チケット、未達 Exit Criteria、ゼロベースレビュー結果、残課題）。**Epic クローズは Ticket クローズ以上に影響が大きいため、必ずユーザの明示的な承認を得ること** |
...
flowchart TD
    ...
    R5 -- "なし" --> R7{"ユーザ確認 gate?<br/>（PD-C-5 / PD-C-10 / PD-D-3）"}
...
PD-C-5、PD-C-10、または PD-D-3 に該当する場合、セルフチェック結果 + Director の裏取り結果をまとめてユーザに報告する。承認はユーザの明示的な意思表示（「OK」「y」「yes」「進めて」等）のみ有効。
...
| **ユーザ確認なしに gate（PD-C-5, PD-C-10, PD-D-3）を越えて進んでいる** | 即座に window を止め、ユーザに状況報告して承認を得る。ユーザが window に質問・会話しただけでは承認にならない。pdh-dev が定義する「明示的な意思表示（OK/y/yes/進めて）」のみ有効 |
...
4. PD-C-5 / PD-C-10 / PD-D-3 の gate はチケット/Epic ごとにユーザ承認を得る
...
---
Based on https://github.com/masuidrive/pdh/blob/486db18/skills/tmux-director/SKILL.md
