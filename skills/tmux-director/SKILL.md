...
flowchart TD
    ...
    TD3report -- "入力待ち" --> TD3idle["TD-3.5 フェーズ遷移検証"]
...
**省略不可ステップ:** PD-C-6, PD-C-7, PD-C-9, PD-C-10

**ユーザ確認が必須の gate:**

| Gate | タイミング | 報告内容 |
|---|---|---|
| **PD-C-5 実装承認** | 計画レビュー完了後、実装開始前 | 計画内容（設計判断、ファイル変更計画、E2E テスト手順、懸念事項） |
| **PD-C-10 クローズ** | 目的妥当性確認・完了検証後 | テスト結果、AC 達成状況、実環境動作確認結果、残課題。**特に「既存問題」「対応検討」「スコープ外」と記載された項目は個別に列挙し、対応方針をユーザに確認する** |
...
4. **PDH 逸脱**: 下記チェック観点に該当するパターンを検知

## 現在フェーズの PDH チェック観点
{PDH_PHASE_CHECKS}

共通チェック: テスト未実行で完了報告 / E2E 省略 / AC 無断書き換え
...
### 画面内容の要約
[作業結果、選択肢、AC達成状況、懸念事項など Director が意思決定に必要な情報をすべて含める]
...
flowchart TD
    ...
    R2 --> R3["完了フェーズを特定し Exit Criteria と照合"]
    R3 --> R4{"Exit Criteria 充足?"}
    R4 -- "いいえ" --> R5["window に是正指示 → Monitor 再起動"]
    R4 -- "はい" --> R6{"ユーザ確認 gate?<br/>（PD-C-5 or PD-C-10）"}
    R6 -- "いいえ" --> R7["次フェーズの指示を送信 → Monitor 再起動"]
    R6 -- "はい" --> R8["window にセルフチェックを送信"]
    R8 --> R9["Monitor でセルフチェック結果を待つ"]
    R9 --> R10["ユーザに状況報告し承認を得る"]
    R10 --> R7
```

#### 手順の詳細

**Step 1: pdh-dev を Read して Exit Criteria を把握する**
- `.claude/skills/pdh-dev/SKILL.md` の該当フェーズセクションを読む
- そのフェーズの完了条件・レビュー構成・必須成果物を確認する

**Step 2: note と ticket を Read して検証する**
- `current-note.md` の該当セクション（例: 「PD-C-4. 計画レビュー結果」「PD-C-7. 品質検証結果」）
- `current-ticket.md` の AC（特に PD-C-8/PD-C-9 付近）
- **チケットの規模に関わらず、この検証を省略してはならない。** Monitor の報告だけで判断せず、自分の目で note を確認すること
...
**Step 4: Gate の場合、window にセルフチェックを送信する**

ユーザ確認 gate（PD-C-5 または PD-C-10）に該当する場合、ユーザに報告する **前に** window に以下を送信し、Monitor で結果を待つ:
...
セルフチェック結果を受け取った後、ユーザに状況を報告する。承認はユーザの明示的な意思表示（「OK」「y」「yes」「進めて」等）のみ有効。
