...
```mermaid
flowchart TD
    Start([開始]) --> P1["Phase 1: ターゲット window の決定"]
    P1 --> P1q{"Claude Code window あり?"}
    P1q -- "なし" --> P1new["ユーザに新規 window 作成を促す"]
    P1new --> P1
    P1q -- "あり" --> P1ask["AskUserQuestion で window 選択"]
    P1ask --> P2["Phase 2: 初期化<br/>/clear → /pdh-dev"]
    P2 --> P3["Phase 3: 監督ループ"]
    P3 --> P3send["指示を送信"]
    P3send --> P3monitor["Monitor Agent をバックグラウンド起動"]
    P3monitor --> P3wait["Monitor からの報告を待つ"]
    P3wait --> P3report{"報告の種類"}
    P3report -- "入力待ち" --> P3idle["状況を把握し次の指示を決定"]
    P3report -- "AskUserQuestion" --> P3ask["ユーザに選択肢を提示し承認を得る"]
    P3report -- "エラー" --> P3err["分析し修正指示を送信"]
    P3report -- "PDH 逸脱" --> P3fix["是正指示を送信"]
    P3report -- "タイムアウト" --> P3timeout["Monitor を再起動"]
    P3idle --> P3gate{"ユーザ確認が必要?"}
    P3ask --> P3send
    P3err --> P3send
    P3fix --> P3send
    P3timeout --> P3monitor
    P3gate -- "はい" --> P3confirm["ユーザに報告し承認を得る"]
    P3confirm --> P3send
    P3gate -- "いいえ" --> P3send
```
...
flowchart TD
    ...
    M2 --> M3{"終了状態を検知?"}
    M3 -- "入力待ち / AskUserQuestion<br/>/ エラー / PDH 逸脱" --> M4["Director に報告"]
    M3 -- "まだ作業中" --> M5{"最大回数に到達?"}
    M5 -- "いいえ" --> M1
...
### 3-2. Monitor Agent の起動

指示送信後、Sonnet Agent をバックグラウンドで起動する:
...
### 3-3. Monitor 報告を受けた後の Director の行動
...
### 3-4. AskUserQuestion への応答（Director が直接行う）
