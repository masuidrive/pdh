...
4. **PDH 逸脱**: 以下の PDH 遵守チェックリストに違反するパターンを検知した場合

## PDH 遵守チェックリスト（各キャプチャで該当フェーズを確認）
- [ ] PD-1: チケットの AC と依存関係を確認しているか
- [ ] PD-2: 十分な調査を行っているか
- [ ] PD-3: E2E スモークテスト手順が計画に含まれているか
- [ ] PD-4: DA×2 + Engineer + codex のレビューが全て実行されているか（codex スキップは逸脱）
- [ ] PD-5: 計画に沿って実装しているか、リードが直接コード編集していないか（TeamCreate 使用）
- [ ] PD-6: DA×2 + CR×2 + codex のレビュー実行、テスト全件パス（SQLite + PostgreSQL + Frontend の3種）、実環境での動作確認
- [ ] PD-7: AC 全項目の達成確認
- [ ] テスト未実行で完了報告していないか
- [ ] E2E スモークテストを省略していないか
- [ ] AC 未達でのクローズを試行していないか
- [ ] ビルド成功だけで実環境テストを省略していないか
- [ ] AC を無断で書き換えていないか
- [ ] docs/wiki の見直しが必要な変更で見直しを省略していないか
