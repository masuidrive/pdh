...
3. ページ本体の `scrollWidth - clientWidth` が `0` で、**かつ画面の外へ右端・左端が出ている要素が無い**ことを確認する。⚠ `html, body` に `overflow-x: clip` を当てていると、要素がはみ出しても `scrollWidth` は育たない — 要素の矩形も見る（実測 2026-08-26: clip の効いた board に幅 3000px の要素を入れても `documentElement` の差は 0 のままだった）。
...
**この検査を手で書き直さない。**内容に依らず機械で見られる範囲は [tools/](tools/README.md) の `check.js` が実行する（幅と言語は設定で渡す。反証つき）。組み立て — kit の CSS / JS の埋め込み、画像の data URI 化、日本語の禁則対策 — は同じ場所の `build.py` が行う。⚠ **どちらも «内容に依らず壊れているもの» だけを見る。**節・カード・AC の数や構成は検査しない（board の形は案件ごとに違うため）。
