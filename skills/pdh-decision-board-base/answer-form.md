...
⚠ **`data-label` は必須。**貼り戻し文に入る短い名前である。**これが無いと、カードの本文が丸ごと貼り戻し文に入る**（`textContent` を拾うため）。**選択 1 件で 300 字を超える貼り戻し文になる。**
...
**逆に置くと、推奨のバッジが «選ばれている印» に読める。**

**「選ぶ」というボタン文字は置かない。**空の丸が «押せば選べる» を、塗られた丸が «選んでいる» を運ぶので、文字は要らない。

⚠ **塗り（背景色）は «選ばれている» だけの印にする。**推奨カードにも塗りがあると、**どちらが選択中か見分けられない**。推奨であることは、右のバッジと枠線の色で足りる。選択中のカードには**太い枠・左の帯・塗り・塗られた丸**を付け、**同じ判断の他のカードは薄くする**。
...
- 貼り戻し文の短い名前は **`data-label` から**取る。カードを選択肢にすると `textContent` は «カード全部» になり、本文が丸ごと貼り戻し文へ入るため。
...
**コピーは送信があっても消さない。**送信できない場面（ログインが切れた・ホストが落ちた）で返す手段が無くなるため、予備として必ず残す。ただし⚠ **送信が出ている board では、コピーを «予備» の見た目に落とす** — 同じ強さで並べると «どちらでもよい 2 つ» に見え、押す先が決まらない。`kit/board.js` が送信ボタンを足したときだけコピーへ `data-copy-secondary` を付け、`kit/board.css` が枠と地を落とす。⚠ **送信の無い board では落とさない** — そこではコピーが唯一の返し方である。
