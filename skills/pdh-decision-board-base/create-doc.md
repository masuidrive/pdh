...
- **目次** — 節が 5 つを超える board に置く。`<div class="layout"><nav class="toc" id="toc"><button class="toc-t" type="button" id="toct">目次</button><ol><li><a href="#節id">節名</a></li>…</ol></nav><div class="content">…全節…</div></div>`
  で本文を包む。⚠ **アンカーの宛先を見出しではなく `<section id>` にすること** — 見出しに
  向けると選択肢が宛先の中に入らず、**判断のある節が «判断なし» に見える**（`check-static.sh`
  の「ページ内参照」が拾う）。
...
1. **本文中の各 AC 参照と範囲参照が、実在する原文の id を指しているか**を検索で確かめる（`check-static.sh` の「ページ内参照」が拾うが、AC 番号の付け違いは拾わない）。
2. **開始・終了タグの個数と見出し数を、編集前後で比較する。**正規表現で節を一括置換した場合は、AC 数と回答 `data-q` の数も比較する。
3. **目次のアンカーが `<section id>` を指しているか** — `check-static.sh` の「ページ内参照」が拾う。見出しに向けると選択肢が宛先の中に入らず、**判断のある節が «判断なし» に見える。**

HTML は閉じタグが欠けても描画される。残りの本文が誤った親要素に包まれるため、**目視だけで構造を判断しない** — だから 2 を数で比べる。

⚠ **sticky `h2` の重なり、`<details>` の開閉、表の横 scroll、`getComputedStyle` — これらは書かない。**skill の中からブラウザは開けず、どれも `kit/board.css` が保証している範囲である（[kit/README.md](kit/README.md)「kit が保証している挙動」）。**kit を変えたときだけ**、PDH repo 側の `scripts/check-board-render.sh` が実ブラウザで確かめる。
