...
洗い出した未確定の項目（洗い出しの手順は gate skill が定める）を 3 つに分ける。
...
**`N` は «承認者が独立に返答できる指示の単位» の数である。**選択肢の数でも、成果物の数でも、未確定の項目の数でもない。**判断数そのものも疑い、書き手が決められる項目を数に含めない。**
...
決定サマリーの 5 行へ多数の判断を列挙しない。サマリーにはグループ名、今回承認する段階、未承認で残る段階を書く。1 回の board で認知負荷を下げられない場合は、ticket の契約を分割せず、同じ ticket に対する複数回の承認状態として扱う。
...
`N = 0` では 2 行目を承認文にする（1 行目の目的は `N = 0` でも書く）。多数モードでは個別判断ではなくグループと承認段階を示す。
...
背景: <この判断がどの Open Question・どの要望から来たか>
推奨: <案の記号と名前 + 選ぶ理由 1 行>
...
選択肢 B — <別案。同じ 4 欄を同じ精度で。薄く書けば誘導になる>
...
⚠ **ただし «前回の回答» からの差分は、畳んで必ず置く**（ユーザ指示 2026-08-23）。承認者が一度回答を返し、その指示を受けて作り直して再発行した board では、**承認者は「自分が出した指示が通ったか」を確かめる。**それが board に無ければ、承認者は**板を最初から読み直して差分を自分で復元する**ことになる。畳みの題は「前回の回答『<返ってきた指示>』に対して変えたこと」のように、**何に対する差分かが分かる形**にする。基準は**前の版ではなく前の回答**である — 版は書き手の都合で何度も上がるが、承認者が確かめたいのは**自分の指示に対する応答**だけである。回答がまだ無い board には、この畳みは要らない。

⚠ **これは «目的を書く» と «差分を畳む» の両方でひと組である。**目的を足しても差分が主線に残れば、読み手は依然として前回を復元しようとする。逆に差分を畳んでも目的が無ければ、**結果だけがあって理由の無い板**になる。

**この規則は、次の実測のあとに書いた。**2026-08-23、close 板を «実装を撤去して閉じるか» という結論から書き起こした結果、**「この ticket が何を作ろうとしていたか」がどこにも無い板**になった。指摘 5 件を「1 件も直していない」とだけ書いたため、**なぜ直さず捨てたのかも主線から落ちていた**（畳みの中にはあった）。発注者から「単体もこれ読んで分かる？」と問われて発覚した。**書き手は経緯を知っているので、経緯の続きとして書いてしまう。**
...
⚠ **そうならないよう、board の冒頭で線を宣言する。**「この board の数は目安である。正確に書いたのは «判断が変わる事実» のほうである」と書き、何を正確に書いたかを並べる。**宣言があれば、検査の reviewer も数の不一致を指摘しない。**
...
（ユーザ指示 2026-08-22:「板の精度は私の判断が間違えない程度、行数や個数に誤差があっても判断は変わらない。コミット id とかも書く必要ないよね。せめてブランチ名じゃない？ソース読まないって言ってるんだからそこの正確性に意味はない」）
...
動詞を書いたら、その 1 文に主語と目的語があるか確認する。とくに比喩の動詞と、指示対象のない「もの」「こと」「部分」「ところ」を避ける。見出しと結論行は圧縮で欠落しやすいため、単独で意味が閉じるか確認する。
