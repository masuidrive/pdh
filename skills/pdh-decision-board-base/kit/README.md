...
| `board.js` | 回答フォームの動作（選択・メモ・進捗・貼り戻し文の生成・3 段コピー・送信 hook・進捗バッジの移動）。文書とデッキで同じ 1 つ |
...
- **進捗バッジ（`.answer-progress`）は押せる。**`board.js` が結び付けるので、**board 側は何も書かない** — `data-scroll-target` を書かせると、判断を足すたびに書き手が更新することになる。押すと、**まだ答えていない最初の判断の節**へ運ぶ（選択もメモも無いものを «未回答» と数える。進捗・目次の ✓ と同じ数え方）。全部答えてあれば**送信ボタン**へ運び、貼り戻し欄と一緒に見えるよう中央へ寄せる。⚠ 運ぶ先は**選択肢カードの節**であって `.answer-set` ではない — あちらはメモ欄の入れ物で、選択肢より下・節の外に置かれる。
