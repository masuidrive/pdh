...
手順 8 から戻れるのは «決められなくする» 指摘と形の崩れだけで、**戻り先は欠陥を持ち込んだ手順である**（点線の辺）。⚠ **戻って直しても、検査（手順 8）は回し直さない** — [final-check.md](final-check.md)「出す条件」のとおり reviewer は 1 回だけである。**例外は構成を組み直したときだけ**（点線の «構成の組み直し» の辺）で、そのときは 4〜8 を未完了へ戻して 8 をもう 1 回だけ回す。
...
**AC は ticket を書く時点で承認者の言葉になっている**（ticket 側の規則が正であり、この skill は複製しない）。**board はそれをそのまま出す。**
