...
このファイルは、gate と媒体に依存しない判断ボードの共通規則である。手順と読む順は [SKILL.md](SKILL.md) にある。
...
1 問そのものは [SKILL.md](SKILL.md)「唯一の検査」にある。
...
- ⚠ **その判断は、目で見て決めるものか。**画面の見た目・配置・操作の有無・出力の形を選ばせるなら、**承認者は像が無いと選べない**（「判断カードの型」の «出来上がりの像は具体物で置く»）。⚠ **ここで判定する理由は、像を撮るのに段取りが要るからである** — アプリを build して起動し、データを入れ、ブラウザを動かす。**書き手が subprocess で、アプリを起動できないこともある。**手順 5 で気づくと、**像の無いまま出すか、全部やり直すかの二択になる。**像が要ると判定したら、**誰がいつ撮るかをここで決めてから書き始める。**
...
- **承認者にしか決められない** — board の判断にする。**不可逆な決定、プロダクトとしての価値判断、費用の負担、既知のリスクを残す決定、信頼境界を動かす変更**が該当する。**実装方式の差でも、出力・監視・運用手順が変わるなら承認者の判断にする。** **どれにも当たらない実装方式・命名・内部構造は、書き手が決める。**
    - ⚠ **見た目の差は、それだけでは «価値判断» にならない。**⚠ **利用者ができること・費用・不可逆性のどれも変わらない見た目の差は、書き手が決めて理由を note に残す。**「押せないボタンを出すか出さないか」「導線をどこに置くか」は、**推奨を持っている書き手が決められる。**⚠ **判断にしてよいのは、その見た目の差で «誰が何をできるか» が変わるとき**（操作を隠して到達できなくなる、名前が契約として外部に出る、等）である。
    - ⚠ **「利用者が見る結果が違う」を判断化の根拠にしない。**画素が 1 つでも違えば «見る結果» は違うので、この語を字義どおり取ると**すべての見た目が承認者へ回る。**上の 1 問（Why の達成・AC・**利用者ができること**）で篩う。
...
- **出来上がりの像は、文章ではなく具体物で置く。**承認者は文章から画面や出力を想像できない。 UI の変更なら、実装前でも画面の試作を撮って選択肢の隣に埋める（撮り方は下の「出来上がりの像の作り方」）。API・CLI・データ・ログなら、各案が返す実例の文面を置く。**全案で同じ入力 1 件を使う** — 入力が違うと承認者は案を比較できない。**案ごとに結果が違うなら、案の数だけ像を作る。**そのときは同じ入力・同じ画角・同じ幅でそろえ、**差が現れる部位を含めて**撮る（同条件でも、変化の薄い部位を写すと「前後同じ」に見える — 差が小さいならその事実も caption に書く）。⚠ **幅は «承認者がその画面を実際に見る幅» で撮る。**書き手が検査に使った幅で撮らない — board を読む環境（PC か、スマホか）を確かめ、その幅で撮る。**狭い幅で撮った像を広い画面の読み手へ出すと、図の箱の中に小さな画像が浮き、細部も読めない**。読む環境が分からなければ承認者に聞く。**どの像がどの案かを像の隣に明記し**、変更前（現在）の像も基準として 1 枚置く — 並べて見たときに差がそのまま判断材料になる形にする。**承認された像は「合意済みの画像」になり**、実装後の確認で実物と突き合わせる。 `N = 0` では、承認対象の変更後の像をゴールの面に置く。
- **「安い」「面倒」だけで退けない。**書き手が測れることを「先に測る案」として置かない。
- `N = 0` では別案を捏造しない。⚠ **ただし «承認する / 承認しない» の 2 つは選ばせる**（上記）。「承認しない」を選んだときの修正指示・差し戻し理由は、コメント欄が本体になる。

### 出来上がりの像の作り方

**別の HTML を書き起こさない。アプリを起動し、提案する要素を実画面の DOM へ差し込んでから撮る。**書き起こした試作は、余白・字体・テーマの色が実装後とずれる。**ずれた像が「合意済みの画像」になると、実装後に «合意した絵と違う» が起きる。**差し込みならアプリ本体の CSS がそのまま効くので、承認者が見た像と実装後の画面が一致する。

- **変更前と変更後を、同じセッション・同じ幅・同じテーマで連続して撮る。**差し込む前に 1 枚、差し込んだ後に 1 枚。条件をそろえるために、この 2 枚の間でページを開き直さない。
- **差し込む要素は提案そのものにする。**列の名前・条件の出し方・語は、その board が聞いている判断の回答で変わる。だから**その像がどの選択肢を選んだ姿かを、判断カードに列挙する**（「この像は判断 3 の A と判断 4 の A を選んだ姿である」の形）。列挙が無いと、**承認者が像を見て承認したつもりの内容と、実際に承認した選択肢がずれる。**
- **値は実データから採る。**採れない値は、例であることを像の隣に書く。

**試作の印は、像の隣ではなく像の中に焼く。**像は board の外へ持ち出される — 貼り付け、転送、画面の再撮影。**そのとき隣の文字は失われ、試作が «実装済みの証拠» として出回る。**「試作 — 未実装」を、本文を隠さない角へ焼き込む。差し込みで撮るなら、この印も同じ差し込みで足せる。⚠ **像の隣に書くのをやめるのではない。**像の中・像の隣・alt の 3 か所に書く。

## Acceptance Criteria の書き方〔手順 4〕

**AC は ticket を書く時点で承認者の言葉になっている**（ticket 側の規則に従い、この skill は複製しない）。**board はそれをそのまま出す。**

- **読んで意味の取れない AC が来ても、board で言い換えて辻褄を合わせない。**board を書き進める前に ticket を直し、直した原文を出す。**board 側だけを読める形にすると、ticket の AC は実装の言葉のまま残る** — そして board を作らない場面（会話での確認、レビュアーへの提示、close 報告）では読めないままになる。
...
⚠ **「畳む」は、開く操作をしないと本文が見えない状態にすることである。**節を分けて後ろへ置くのは «畳んだ» ではない。**下へ続いている限り、承認者はスクロールで全部読む。**HTML なら閉じた `<details>`、Markdown なら折り畳み記法にする。⚠ **実測: 裏付け 5,400 字を «裏付け» 節に置いただけで畳まず、開いた瞬間に 13,000 字が見える board が出た**（`evals/examples.md`）。

⚠ **主線が 1 画面に収まらなくなったら、それが上の 1 問（「読まなくても同じ判断を下せるか」）を全段落へ当てる合図である。**⚠ **量に目標値は置かない** — 目標を置くと、書き手はそこまで埋める。**短い board は短いままでよい。**

⚠ **削るのは «読む必要» であって «情報» ではない。**承認者はコードを読めるし、書き手より広く知っている。**足りないのは時間だけである。**背景・ファイルの抜粋・測定の生データは、**畳んで残す** — 疑ったときに自分で確かめる経路が無いと、承認者は書き手の断定を信じるしかなくなる。

⚠ **読む側の時間は、書く側の時間より桁違いに高い。**だから量を減らす手段は «削る» ではなく **«書き手が余分に働く»** である — 測る、決める、畳む、要約を先に置く。⚠ **«調べれば分かる» を主線に並べて渡すのは、桁違いに高い時間へ肩代わりさせることである。**
...
- 読み手の技術水準は上級エンジニアとする。SQL・schema・API の応答・設定ファイルの断片は、board に貼って必要な説明を添えれば読める。**避けるのは技術的な内容ではなく、board の外を見に行かせること**である。
...
⚠ **board が厚くなる最大の原因は、説明の書きすぎではなく判断を立てすぎたことである。**判断を 1 件立てると、案ごとに内容・ゴールへの効き・出来上がり・得失・覆る条件・ticket への反映が要る。**案 3 つなら 15 ブロック増える。**

**Completed Staff Work は «短く書け» ではなく «選ばせるな» である。**幕僚は案を並べて上官に選ばせず、**1 つの推奨まで詰め切って持っていく。**⚠ **量を削りたくなったら、まず「判断の分類」へ戻って `N` を減らせないかを見る。**文を削るのは、そのあとである。

### 短さも長さも、それ自体は合否基準ではない

短くした結果、承認者が指示対象や根拠を復元するなら完成条件 ① に反する。斜め読みは文を削ることではなく、見出しと強調だけを拾って判断の筋が通るようにすることで実現する。

⚠ **長さも同じだけ危ない。**完成条件は対であり、**② «承認者が、その判断に使わないものを読まされない» を破った board は、① を満たしていても未完成である。**⚠ **量は «決めることの大きさ» に釣り合わせる** — 見た目を 1 つ選ぶ board と、外部契約を変える board が同じ厚さになることはない。

**判定は行数ではなく、1 段落ずつ次の 1 問で行う**（数えられる基準を置くと、書き手は基準の側へ書き方を寄せる）。

> **この段落を読まなくても、承認者は同じ判断を下せるか。**下せるなら主線から外し、裏付けへ落とすか消す。

⚠ **「あとで疑われたときのために」は主線に残す理由にならない。**それは裏付けの役目である。
...
**発行先はプロジェクト設定が定める。**設定がない場合は、ticket の一時領域または安全な作業領域のファイルへ書き、会話に絶対 path を示す。発行手段は engine の能力で選ぶ — 描画された artifact を発行できる engine はそれを使い、できない engine は同じ構造をファイルへ書いて path を渡す。**要件は材料とその読みやすさであって、描画の機構ではない** — 豊かな機構が使えないことを理由に材料を省かない。公開 URL や共有ツールへ新しく発行すると取り消せない場合は、試作やサンプルを発行する前に承認を得る。機微な情報には risk overlay を優先する。
