...
#### 1. ユーザケーステスト (PM が自ら実施、subagent 委譲不可)

**PM 自身** が Epic の Outcome / Exit Criteria に基づき、主要ユーザジャーニーを実環境で端から端まで実行する。**この工程は subagent / LLM agent に委譲しない** — agent はテストケースの定型実行は得意だが「UI の違和感」「操作フローの不自然さ」「期待値との微妙なズレ」は人間 (PM がユーザ代理) が触って初めて見える:

- **実環境の起動**: プロダクトとして実動する環境を用意する (ローカルサーバ、デモモード、ステージング等)
- **UI がある場合は実際に開いて操作する**: ブラウザ自動化 CLI または実ブラウザで画面遷移・click・form 入力を実際に行い、アニメーション・レイアウト・エラー表示を目視
- **CLI / API のみの場合**: 実コマンド / 実 HTTP リクエストで end-to-end のフローを実行する
- **エッジケース**: 空データ / 不正入力 / 長文 / 権限境界 / race condition 再現を意図的に試す
- **検証のための一時的な改変は許可**: 認証バイパス、特殊 seed (edge 再現用)、一時的な log 挿入など。影響範囲を意識し、検証後は改変を revert する
- **記録は一人称 note** として `epics/done/<slug>/verification.md` に残す。「何を試したか・何が見えたか・感想・気になった点」を PM 視点で書く。スクリーンショット等の artifact も残す

問題が見つかった場合:

- **修正が必要 (Critical / Major 相当)** → 追加 Ticket を切って対応。完了後 PD-D-3 を再実行
- **軽微・改善案** → `docs/future-list.md` に記録
- **過去 Epic に起因する regression** → 別 ticket で bundle して修正
