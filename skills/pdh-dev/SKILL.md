...
PDH は **ヒエラルキー** である。Epic は Product Brief のゴールへ導くために書き、Ticket は Product Brief と Epic のために書く。常に **「誰の」「どんな問題を解くか」** を意識し、そこに貢献するために Epic / Ticket を書くこと。これが Epic・Ticket を書く際の最重要事項である。
...
- `ticket.sh` が Ticket ごとに `feature/<ticket-name>` ブランチを自動作成し、close 時に main にマージする。**Epic ブランチは作らない**
...
```mermaid
flowchart TD
    C0["C0. 開始前チェック<br/>current-ticket.md / current-note.md 確認"] --> C1a
    C1a["C1-a. 変更対象の調査<br/>Explore Agent 並行実行"] --> C1b
    C1b["C1-b. 計画立案<br/>current-note.md に記録"] --> C2
    C2["C2. 計画レビュー<br/>DA×2 + Engineer + codex 並行"] --> C2q{"Critical/Major?"}
    C2q -- "あり" --> C1b
    C2q -- "なし" --> C3
    C3["C3. 実装<br/>TeamCreate で Engineer 並行"] --> C4
    C4["C4. 品質検証<br/>DA×2 + CR×2 + codex 並行"] --> C4q{"Critical/Major?"}
    C4q -- "あり" --> C4fix["修正 → テスト"] --> C4
    C4q -- "なし" --> C45
    C45["C4.5. 目的妥当性確認<br/>成果物がチケット目的を十分に達成しているか"] --> C45q{"不足あり?"}
    C45q -- "あり" --> C3
    C45q -- "なし" --> C5
    C5["C5. 完了処理<br/>docs更新 → commit → ユーザ確認 → close"]
```
...
3. **定期報告**: レビューラウンドが 3 の倍数に達したとき
4. **デッドロック**: 同じ指摘が 2 回修正しても解消しないとき

## 中止フロー

Epic / Ticket を中止する場合:
1. frontmatter に `cancelled_at` を追加する
2. 本文に中止理由を記録する
3. `done/` に移動する（`done/` 内のファイルは消さない。判断の履歴として残す）
...
3. **再レビューの規模は初回より小さくてよい** — 修正箇所に焦点を当てた 2 名程度の再レビューで十分（初回のフルレビューを毎回繰り返す必要はない）
4. **3 ラウンド目に入ったらユーザに相談する** — 同じ指摘が解消しない場合はデッドロックの可能性がある
...
最低限必要なセクション:
- **Summary**: この Epic で達成したいこと
- **Scope**: 含めるもの
- **Exit Criteria**: この条件を満たしたときに閉じる

任意セクション（必要に応じて追加）:
- **Problem** / **Outcome** / **Non-goals** / **Dependencies** / **Tickets** / **Related Links**
...
以下を並行実行してレビュー:
- **DA(Sonnet)×2**: スコープの過不足、Exit Criteria の曖昧さ、product-brief.md との矛盾
...
- Context に Epic へのリンクを含める
- Acceptance Criteria は観察可能な振る舞いで書く
...
1. `current-ticket.md` が存在するか確認する
   ...
   - **存在する場合**: 内容を読んで作業を続行
   - `./ticket.sh close` 時に `current-ticket.md` の内容がコミットメッセージに使われる。Implementation Notes や設計判断は ticket に残すこと
...
| セクション | 記録タイミング | 内容 |
|---|---|---|
| **C1. 調査結果** | C1-a 完了時 | Explore Agent の調査結果を統合 |
| **C1-b. 計画** | C1-b 完了時 | 実装計画（タスクリスト・ファイル所有権・テスト計画） |
| **C2. 計画レビュー結果** | C2 完了時 | レビュー指摘と対応結果 |
| **C4. 品質検証結果** | C4 完了時 | レビュー指摘と対応結果 |
| **Debug Log** | 随時 | 調査中のメモ、デバッグ情報 |

**必須ルール:**
- **空セクションを残さない** — 該当フェーズを実施したら必ず結果を記録する。スキップした場合は理由を 1 行書く（例: `変更がコード外のため C4 スキップ`）
- セッションをまたぐ引き継ぎ資料として機能させるため、判断の根拠や却下した代替案も記録する

### C1. 計画作成

#### C1-a. 変更対象の調査（Agent 並行実行）
...
調査結果は `current-note.md` の「C1. 調査結果」セクションに記録する。
...
> **前提条件**: C1（計画）と C2（計画レビュー）が完了していること。
...
以下を並行実行する:
- **DA(Sonnet)×2**: セキュリティ脆弱性、設計上の論理バグ
- **CR(Sonnet)×1 + CR(Opus)×1**: コード品質、レースコンディション、認証チェック漏れ
- **codex×1**: 致命的な点のみ指摘
...
C4（コード品質）完了後、クローズ前に **成果物がチケットの目的を十分に達成しているか** を最終確認する。
C4 は「コードが正しいか」を見るが、C4.5 は「正しいコードが十分な量あるか」を見る。

確認観点:
1. **Acceptance Criteria の実質的達成**: 形式的に AC を満たしているだけでなく、AC の意図（Why）を満たしているか
2. **成果物の十分性**: テストなら「主要フローが網羅されているか」、機能なら「ユーザが実際に使える状態か」
3. **抜け漏れの棚卸し**: 実装した機能/テスト一覧を列挙し、明らかに不足しているものがないか確認

不足がある場合:
- **このチケット内で対応すべきもの**: 追加実装 → テスト → C4.5 再確認
- **別チケットに切り出すべきもの**: `docs/future-list.md` またはチケットとして記録

### C5. Ticket 完了処理

1. `current-ticket.md` の Acceptance Criteria を一つずつ確認し、各項目に `[x]` チェックをつける
2. **AC チェックの裏取り**: Review Agent（Sonnet×2）を並行 spawn し、各 AC 項目が実際に達成されているかコード・テスト結果・ノートを読んで検証させる。リードの自己判断だけで「達成済み」としない。NOT VERIFIED が返った項目は証拠を補完してから進む
3. `/update-docs` スキルを実行（設定している場合）
4. AC チェック済みの ticket ファイルを含めて `/commit` スキルでコミット
5. `./ticket.sh list` で Epic の残り TODO Ticket を確認する
6. **クローズ前にテキストで以下を報告する**（AskUserQuestion に詰め込まず、先に読める形で出す）:
   - **確認手順**: ユーザが自分で動作確認する方法（URL・curl コマンド・操作手順・期待結果）
   - **懸念事項・残課題**: 既知の制限、スコープ外にした項目、future-list に追記した項目
   - **作業サマリ**: 主な変更を 3-5 行で
7. テキスト報告後、AskUserQuestion で「クローズしてよいか？」を確認（Epic 残チケット状況も含める）
8. 承認後 `./ticket.sh close` でチケットをクローズ
9. ユーザの Epic 完了判断に応じて:
...
> C5 でユーザが「Epic 完了」と判断した場合に、ゼロベースレビュー用チケットの中でこのフローを実行する。
...
以下を並行実行する:
- **DA(Sonnet)×2**: 「今の実装を白紙にして再設計するなら何を変えるか」— アーキテクチャ・データモデル・API設計の改善点
- **CR(Sonnet)×1 + CR(Opus)×1**: 「コードベース全体を見て、技術的負債や構造上の問題はないか」— 重複コード・責務の混在・拡張性の課題
...
| ロール名 | 用途 | 責務 |
|---|---|---|
| **DA (Sonnet)** | Epic・計画・品質レビュー | 設計・仕様への建設的批判 |
| **CR (Sonnet/Opus)** | 品質レビュー | コード品質・セキュリティ検証 |
