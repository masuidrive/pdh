...
- `ticket.sh` が Ticket ごとに `feature/<ticket-name>` ブランチを自動作成し、close 時に main にマージする
- Epic が大きく、Ticket 単位のマージで main が中間的に壊れる場合は Epic ブランチを切ってよい（frontmatter の `branch` に記載）
...
flowchart TD
    ...
    C2["C2. 計画レビュー<br/>Design Advisor×2 + Engineer + codex 並行"] --> C2q{"Critical/Major?"}
    C2q -- "あり" --> C1b
    C2q -- "なし" --> C3
    C3["C3. 実装<br/>TeamCreate で Engineer 並行"] --> C4
    C4["C4. 品質検証<br/>Design Advisor×2 + Code Reviewer×2 + codex 並行"] --> C4q{"Critical/Major?"}
...
- **プロセス要件**（C2レビュー済み、テストパス等）→ note の「プロセスチェックリスト」セクションに書く。C5 完了処理時にチェックする
...
以下を並行実行してレビュー:
- **Design Advisor(Sonnet)×2**: Outcome の明確さ、スコープの過不足、Exit Criteria の曖昧さ、product-brief.md との矛盾
...
| **C2. 計画レビュー結果** | C2 完了時 | レビュー指摘と対応結果。**「残存 Critical/Major なし」の記録がプロセス通過の証跡になる** |
| **C4. 品質検証結果** | C4 完了時 | レビュー指摘と対応結果。**「残存 Critical/Major なし」の記録がプロセス通過の証跡になる** |
| **Debug Log** | 随時 | 調査中のメモ、デバッグ情報 |

**必須ルール:**
- **空セクションを残さない** — 該当フェーズを実施したら必ず結果を記録する。スキップした場合は理由を 1 行書く（例: `変更がコード外のため C4 スキップ`）
- **プロセスを繰り返すときはチェック項目を追加する** — 問題修正などで C2/C4 等を再実行した場合、プロセスチェックリストに 2 回目以降の項目を追記する（上書きしない）。全ラウンドの証跡を残す
...
以下を並行実行する:
- **Design Advisor(Sonnet)×2**: 設計上の矛盾、見落としたリスク
...
以下を並行実行する:
- **Design Advisor(Sonnet)×2**: セキュリティ脆弱性、設計上の論理バグ
...
1. `current-ticket.md` の **プロダクト AC**（振る舞い）を一つずつ確認し、各項目に `[x]` チェックをつける
2. **プロセスチェックリストの確認**: `current-note.md` の「プロセスチェックリスト」セクションを一つずつ確認し、各項目に `[x]` チェックをつける
3. **AC チェックの裏取り**: Review Agent（Sonnet×2）を並行 spawn し、各 AC 項目が実際に達成されているかコード・テスト結果・ノートを読んで検証させる。リードの自己判断だけで「達成済み」としない。NOT VERIFIED が返った項目は証拠を補完してから進む
4. `/update-docs` スキルを実行（設定している場合）
5. AC チェック済みの ticket ファイルを含めて `/commit` スキルでコミット
...
以下を並行実行する:
- **Design Advisor(Sonnet)×2**: 「今の実装を白紙にして再設計するなら何を変えるか」— アーキテクチャ・データモデル・API設計の改善点
...
| ロール名 | 用途 | 責務 |
|---|---|---|
| **Design Advisor (Sonnet)** | Epic・計画・品質レビュー | 設計・仕様への建設的批判 |
