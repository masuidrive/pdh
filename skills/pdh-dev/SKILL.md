...
5. Epic ブランチを main にマージする: `git checkout main && git merge epic/<name> --no-edit`。コンフリクトがあれば解決し、`scripts/test-all.sh` で全テストパスを確認する。Epic の `branch` が `main` の場合はこのステップをスキップする
