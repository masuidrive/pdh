...
```mermaid
flowchart TD
    PD1["PD-1. 開始前チェック<br/>current-ticket.md / current-note.md 確認"] --> PD2a
    PD2a["PD-2a. 変更対象の調査<br/>Explore Agent 並行実行"] --> PD2b
    PD2b["PD-2b. 計画<br/>current-note.md に記録"] --> PD3
    PD3["PD-3. 計画レビュー<br/>Devil's Advocate×2 + Engineer + codex 並行"] --> PD3q{"Critical/Major?"}
    PD3q -- "あり" --> PD2b
    PD3q -- "なし" --> PD4
    PD4["PD-4. 実装<br/>TeamCreate で Engineer 並行"] --> PD5
    PD5["PD-5. 品質検証<br/>Devil's Advocate×2 + Code Reviewer×2 + codex 並行"] --> PD5q{"Critical/Major?"}
    PD5q -- "あり" --> PD5fix["修正 → テスト"] --> PD5
    PD5q -- "なし" --> PD6
    PD6["PD-6. 目的妥当性確認<br/>成果物がチケット目的を十分に達成しているか"] --> PD6q{"不足あり?"}
    PD6q -- "あり" --> PD4
    PD6q -- "なし" --> PD7
    PD7["PD-7. 完了処理<br/>docs更新 → commit → ユーザ確認 → close"]
```
...
- **プロセス要件**（PD-3レビュー済み、テストパス等）→ note の「プロセスチェックリスト」セクションに書く。PD-7 完了処理時にチェックする
...
| セクション | 記録タイミング | 内容 |
|---|---|---|
| **PD-2. 調査結果** | PD-2a 完了時 | Explore Agent の調査結果を統合 |
| **PD-2b. 計画** | PD-2b 完了時 | 実装計画（タスクリスト・ファイル所有権・テスト計画） |
| **PD-3. 計画レビュー結果** | PD-3 完了時 | レビュー指摘と対応結果。**「残存 Critical/Major なし」の記録がプロセス通過の証跡になる** |
| **PD-5. 品質検証結果** | PD-5 完了時 | レビュー指摘と対応結果。**「残存 Critical/Major なし」の記録がプロセス通過の証跡になる** |
| **プロセスチェックリスト** | PD-7 完了処理時 | プロセス要件のチェック（PD-3/PD-5 解消、テストパス、E2E 確認等） |
| **Debug Log** | 随時 | 調査中のメモ、デバッグ情報 |

**必須ルール:**
- **空セクションを残さない** — 該当フェーズを実施したら必ず結果を記録する。スキップした場合は理由を 1 行書く（例: `変更がコード外のため PD-5 スキップ`）
- **プロセスを繰り返すときはセクション名に回数を付ける** — 問題修正などで同じフェーズを再実行した場合、「PD-3(2回目)」「PD-5(3回目)」のようにセクションを追加する（上書きしない）。プロセスチェックリストも同様に項目を追記する。全ラウンドの証跡を残す。例:
  ```
  ## PD-3. 計画レビュー結果
  （1回目の結果）
...
> **前提条件**: PD-2（計画）と PD-3（計画レビュー）が完了していること。
...
PD-5（コード品質）完了後、クローズ前に **成果物がチケットの目的を十分に達成しているか** を最終確認する。
PD-5 は「コードが正しいか」を見るが、PD-6 は「正しいコードが十分な量あるか」を見る。
...
不足がある場合:
- **このチケット内で対応すべきもの**: 追加実装 → テスト → PD-6 再確認
...
> PD-7 でユーザが「Epic 完了」と判断した場合に、ゼロベースレビュー用チケットの中でこのフローを実行する。
