...
5. **ステップ省略**: 全ステップ実行を推奨する。ユーザが明示的に省略を指示した場合のみ省略可。省略した場合は `current-note.md` 冒頭に記録する。省略不可: PD-5（実装）、PD-6（品質検証）、PD-8（完了処理）
...
- **ファイル別コンテキスト**: 変更対象ファイルごとに、PD-2 の調査結果から直近の設計判断・注意点を 2-3 行でまとめる。Engineer spawn 時にこの情報を渡し、過去の設計判断に反する変更を防ぐ
...
### ユーザ確認: 計画完了 → 実装開始前（PD-4 → PD-5 の境界）

ユーザが明示的に省略を指示しない限り、実装に進む前に以下をテキストで報告し、AskUserQuestion で実装開始の承認を得る:
- **計画内容**: 設計判断、ファイル変更計画、チーム分担
- **E2E テスト手順**
- **懸念事項・リスク**: レビューで出た Minor 指摘や残存リスク
...
**リードは直接コードを書かない。** TeamCreate で実装チーム（PM + Engineer(s)）を作り、計画に従って実装させる。各 Engineer の spawn プロンプトには、担当ファイルの「ファイル別コンテキスト」（PD-3 で作成）を含めること。完了後:
- 自動テスト（全件パス必須）
...
### ユーザ確認: コードレビュー後 → チケットクローズ前（PD-7 → PD-8 の境界）

ユーザが明示的に省略を指示しない限り、クローズ処理に進む前に以下をテキストで報告し、AskUserQuestion でクローズ処理の承認を得る:
...
7. **クローズ前にテキストで以下を報告する**（AskUserQuestion に詰め込まず、先に読める形で出す）:
   ...
   - **作業サマリ**: 主な変更を 3-5 行で
8. 懸念事項・残課題の中にチケット化できるものがあれば、一覧を提示し AskUserQuestion でどれをチケット化するか選択させる。選択されたものは `./ticket.sh new` で作成する
9. テキスト報告後、AskUserQuestion で「クローズしてよいか？」を確認（Epic 残チケット状況も含める）
10. 承認後 `./ticket.sh close` でチケットをクローズ
11. ユーザの Epic 完了判断に応じて:
