...
結果を → 「レビューパターン（共通）」に従って統合する。**Critical / Major の採否と future-list 振り分けの判断は PdM（Opus）を spawn して委譲する**。PdM に `product-brief.md`・Epic・レビュー結果一覧を渡し、以下の分類を判断させる:
...
**PM 自身** が Epic の Outcome / Exit Criteria に基づき、主要ユーザジャーニーを実環境で端から端まで実行する。**この工程は subagent / Codex / QA Engineer に委譲しない** — LLM agent はテストケースの定型実行は得意だが「UI の違和感」「操作フローの不自然さ」「期待値との微妙なズレ」は人間 (PM がユーザ代理) が触って初めて見える:

- **実環境の起動**: local server / demo モード等、プロダクトとして実動する環境を用意する
- **ブラウザ UI は実際に開いて操作する**: agent-browser CLI / ユーザ実ブラウザで URL 移動・click・form 入力を実際に行う。画面遷移のアニメーション・レイアウト・エラー表示を目視
...
- **記録は一人称 note** として `epics/done/<slug>/verification.md` に残す。「何を試したか・何が見えたか・感想・気になった点」を PM 視点で書く。Screenshot も残す
