...
各フェーズ（A1, A2, B1, PD-2〜PD-8）完了時にコミットする。コミットメッセージは `[フェーズ名] 概要` の形式（例: `[PD-3] Planning`）。
...
Epic レビュー（A2）・計画レビュー（PD-4）・品質検証（PD-6）・ゼロベースレビュー（D2）はすべて同じ構造で動く。

```mermaid
flowchart TD
    Q1["Review Agent + codex を並行実行"] --> Q2["結果を union で統合・裏取り"]
    Q2 --> Q3{"Critical/Major 指摘あり?"}
    Q3 -- "あり" --> Q4["修正を反映"] --> Q5["再レビュー実行<br/>（指摘を出したレビュアーが再確認）"]
    Q5 --> Q6["レビュアー別にステータスを記録"]
    Q6 --> Q7{"全レビュアーが<br/>No Critical/Major?"}
    Q7 -- "いいえ" --> Q4
    Q7 -- "はい" --> Q8["完了"]
    Q3 -- "なし" --> Q8
```
...
4. **3 ラウンドごとにユーザに相談する** — 同じ指摘が解消しない場合はデッドロックの可能性がある
...
レビューで発見した問題が今回のチケットの変更によるものでない（既存の問題）場合:
- **同じ Epic のスコープ内で小さい修正**: このチケット内で修正してよい
- **同じ Epic のスコープ内だが大きい修正**: 別チケットに切り出す
- **Epic のスコープ外**: `docs/future-list.md` に記録、またはチケット化を検討
- **セキュリティ上の重大な問題**: ユーザに相談する

このルールは PD-4（計画レビュー）、PD-6（品質検証）、PD-7（目的妥当性確認）すべてに適用する。
...
ドラフト作成後、コミットする（例: `[epic] Create <epic-name>`）。
...
Epic 確定後、コミットする（例: `[epic] Review <epic-name>`）。
...
Ticket 作成・Epic 更新後、コミットする（例: `[ticket] Cut tickets from <epic-name>`）。
...
| セクション | 記録タイミング | 内容 |
|---|---|---|
| **PD-2. 調査結果** | PD-2 完了時 | Explore Agent の調査結果を統合。技術的な懸念事項も記録する |
| **PD-3. 計画** | PD-3 完了時 | 実装計画（タスクリスト・ファイル所有権・テスト計画） |
| **PD-4. 計画レビュー結果** | PD-4 完了時 | レビュアー別ステータステーブル + 指摘と対応結果 |
| **PD-6. 品質検証結果** | PD-6 完了時 | レビュアー別ステータステーブル + 指摘と対応結果 |
...
調査結果は `current-note.md` の「PD-2. 調査結果」セクションに記録し、コミットする（例: `[PD-2] Investigation`）。
...
- **懸念事項への対処**: PD-2 で記録した懸念事項への対処方針を計画に反映する。不確実性が高い場合は AskUserQuestion でユーザに相談する

計画を `current-note.md` に記録し、コミットする（例: `[PD-3] Planning`）。
...
レビュー結果を `current-note.md` に記録し、コミットする（例: `[PD-4] Plan review`）。

### ユーザ確認: 計画完了 → 実装開始前（PD-4 → PD-5 の境界）

ユーザが明示的に省略を指示しない限り、実装に進む前に以下をテキストで報告し、AskUserQuestion で実装開始の承認を得る。**承認はユーザの明示的な意思表示（「OK」「y」「yes」「進めて」等）のみ有効。曖昧な返答の場合は再確認する。**
...
品質検証結果を `current-note.md` に記録し、コミットする（例: `[PD-6] Quality verification`）。
...
4. **懸念事項の確認**: PD-2 で記録した懸念事項が問題化していないか確認する。問題化した場合は影響範囲を評価し対応する

不足がある場合:
- **このチケット内で対応すべきもの**: 追加実装 → テスト → PD-7 再確認
- **別チケットに切り出すべきもの**: `docs/future-list.md` またはチケットとして記録

目的妥当性確認結果を `current-note.md` に記録し、コミットする（例: `[PD-7] Purpose validation`）。

### ユーザ確認: コードレビュー後 → チケットクローズ前（PD-7 → PD-8 の境界）

ユーザが明示的に省略を指示しない限り、クローズ処理に進む前に以下をテキストで報告し、AskUserQuestion でクローズ処理の承認を得る。**承認はユーザの明示的な意思表示（「OK」「y」「yes」「進めて」等）のみ有効。曖昧な返答の場合は再確認する。**
...
5. Acceptance Criteria チェック済みの ticket ファイルを含めてコミットする（例: `[PD-8] Close <ticket-name>`）
...
9. テキスト報告後、AskUserQuestion で「クローズしてよいか？」を確認（Epic 残チケット状況も含める）。**承認はユーザの明示的な意思表示（「OK」「y」「yes」「閉じて」等）のみ有効。曖昧な返答の場合は再確認する**
10. 承認後 `./ticket.sh close` でチケットをクローズ
11. **Epic ファイルの更新**: Epic ファイルの `Tickets` セクションで、クローズしたチケットにチェックを付ける（`- [ ]` → `- [x]`）
12. ユーザの Epic 完了判断に応じて:
