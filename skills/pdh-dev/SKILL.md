...
任意セクション（必要に応じて追加）:
- **Risks** — 想定されるリスクと対応策（技術的リスク、外部依存リスク、スコープリスク）
- **Dependencies** — 他 Epic・外部要因との依存関係
- **Tickets** — Ticket 切り出し後に記載
- **Retrospective** — Epic 完了後の振り返り（D3 クローズ時に記録）
- **Related Links** — 参考リンク
...
| セクション | 記録タイミング | 内容 |
|---|---|---|
| **PD-2. 調査結果** | PD-2 完了時 | Explore Agent の調査結果を統合。識別したリスクも記録する |
...
- **リスク識別**: 技術的リスク（未検証の API、複雑なロジック、パフォーマンス影響）、外部依存リスク（バージョン互換性、API 制限）を洗い出す
...
- **リスク対応策**: PD-2 で識別したリスクごとに対応策（回避・軽減・受容）を決め、`current-note.md` に記録する。リスクが高い場合は AskUserQuestion でユーザに相談する
...
4. **リスク状態の確認**: PD-2/PD-3 で識別したリスクが顕在化していないか確認する。顕在化した場合は影響範囲を評価し、対応策を実行する
...
3. **Retrospective を記録する**: Epic ファイルの Retrospective セクションに以下を記録する:
   - うまくいったこと（再利用したいパターン・判断）
   - うまくいかなかったこと（次回改善したいプロセス・判断）
   - 発見した知見（技術的な学び、ドメイン知識）
4. `closed_at` を追加し `epics/done/` に移動する
