...
PM は実装しない。コード変更は常に Coding Engineerが行う。
...
**Light**: Coding Engineer を 1 つ spawn する。PM は統合、差分確認、追加指示、テスト実行を担当する。

**PM の完了チェック:**

- 自動テスト（全件パス必須）
- CLAUDE.md のテストセクションに従い全スイートを実行する
- 影響レイヤーをカバーするテスト
- 実環境確認
- 実装チームを解散
...
Critical / Major があれば  
**修正 → 自動テスト再実行 → 再レビュー** を行い、全 reviewer role の最新結果が `No Critical/Major` になるまでループする。
