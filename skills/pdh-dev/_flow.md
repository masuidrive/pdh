...
flowchart TD
    ...
    PDC1 --> PDC6["PD-C-6. 実装<br/>1 作業文脈: investigate + implement + tests<br/>commit 細切れ 5+"]
...
- 実装は incremental に 5+ commits に分けて切ること (1 commit = 1 論理単位)
...
- Coding Engineer 追加: `.claude/skills/pdh-coding/SKILL.md を読んでから着手` / commit cadence 5+ / テスト全件 PASS gate / E2E gate / Open Questions protocol / 実装ログを note に追記
...
- 自動テスト全件パス (プロジェクト規約のテスト手順に従い全スイートを実行)
- 影響レイヤーをカバーするテスト
...
5. プロジェクト規約のテスト手順に従い全スイートを最終確認として実行する
