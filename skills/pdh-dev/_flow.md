...
3. **review attempt 追加** → `PDH-review-2` などの子ログとして同じ reviewer role で確認し、全 reviewer の最新結果が `No Critical/Major` になるまで追跡する
...
8. **Surface Observer の起動 (PDH-human-review 直前、必須)**: 外部 surface (UI / HTTP API / SDK / CLI 等) に変更があった場合、consumer 視点の違和感を観察する（実行モデル依存の手段は `_execution-*.md` 参照）
   ...
   - UI / browser surface がある場合は、seed hook 実行後に `agent-browser` 等で主要ユースケースを 1 本以上実行し、snapshot / screenshot / 操作結果または実行不能理由を note と会話に記録する。`agent-browser` を使う場合は直前に `agent-browser --help` を実行する
