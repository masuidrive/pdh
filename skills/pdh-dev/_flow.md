...
6. 最終HEADで`scripts/test-all.sh`を再実行して実出力をnoteへ貼る。後続commitまたはmergeが影響し得る古い証拠は取り直す
7. 必要なticket noteとdocsは直接更新し、通常doc sweepに`pdh-update`を使わない
8. human review直前に外部surfaceをconsumer視点で観察する
...
9. AC check済みticket fileを含めてcommitする
