...
1. WhyとAC
   ...
   - **ACが確定したら、読めるかを«書いていないagent»に測らせる。**`What`冒頭の1文とAC全件だけを渡し（ticket本体・note・実装・この工程の経緯は渡さない）、**「終わると誰が何をできるようになるか」を復元させる。**復元できないACが出たら書き直し、書き直した分だけをもう一度渡す。役割別指示は`pdh-verifying` skill「AC 読み手（復元テスト）」にある
     ...
     - ⚠ **合格は«指摘ゼロ»ではない。**全ACが復元できれば合格で、そこで止める。**指摘へACを足して応えない** — ACを増やす必要が見えたら、それは復元テストの結果ではなく上の洗い出しの漏れなので、そちらへ戻す（ACの増減にはユーザの明示承認が要る）
     - ⚠ **全件の初見は1回だけで、渡し直しても直った確認にはならない**（毎巡が別の読み手のため）。**同じACが2巡続けて復元できなければ、書き直しでは閉じない** — そのACを未確定判断として`PDH-ticket-human-review`の板へ載せ、承認者へ渡す
