...
規則は `PDH-AGENTS.md`「Execution Model」にある。実装での当てはめ方だけをここに書く。

- **指摘を複数直すときは、指摘 1 件 1 項目で一覧に載せる。**直した件数ではなく、**一覧の全項目に決着が付いたこと**で完了を判定する
- **1 つの修正が複数ファイル・複数層に及ぶなら、それも複数作業である**（本体・呼び出し元・テスト・doc）。ファイル 1 つ直したところで完了報告をしない
- **実装中に見つけた «あとで直す» は、その場で一覧へ足す。**note へ書くだけにしない — note は読み返す機会が決まっていない
