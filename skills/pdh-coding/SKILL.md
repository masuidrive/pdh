...
### 隔離された作業ツリーでは、実行できるコマンドの形が制約される

**PDH は ticket ごとの worktree 分離を既定にしている**（`_execution-team.md` / `tmux-director`）ので、これは例外ではなく通常の作業条件である。隔離された session では、**共有 checkout（元の repo ディレクトリ）へ届くコマンドを harness が実行前に拒否する。**拒否されるのは危険なコマンドだけではなく、**行き先を静的に確かめられない形すべて**である。実測で拒否された形:

- 複数のコマンドを `&&` や改行でつないだもの、出力リダイレクトを含むもの
- `git -C <path>` のように作業ディレクトリを引数で移すもの
- 文字列を `eval` に渡すもの（ブラウザ操作 CLI の `eval` サブコマンドを含む）
- `setsid` / `nohup` でプロセスを切り離すもの

**1 コマンド 1 目的に割って書く。**拒否されてから割り直すと、同じ確認を 2 度打つことになる。

⚠ **拒否が続いても「この session ではコマンドを実行できない」と結論しない。**⚠ **隔離は shell の作業ディレクトリではなく session の状態として保持されているので、`cd` では出られない。**出る手段は engine が持つ（Claude Code は `ExitWorktree`、tmux Director 運用では隔離していない window で実行する）。**手段が分からないなら、出られないと報告する前にユーザへ確認する。**
...
- **観測した問題は無視せず記録・分類する**（current ticket で直すか、起票 / 記録のみ / 棄却 のどれにするかを決めるのは PM。判定基準は `PDH-AGENTS.md`「Verification」の Scope boundary）。**観測していない将来問題のための設計はしない**。この対比が境界線。
...
2. **commit message に `ASSUMPTION:` prefix で明示**:
   ...
   ```
3. 完了時 (or 中断時) に **`<RESULT_FILE>` 末尾に `## Open Questions Summary`** を batch で列挙。上位（team: PM / solo: ユーザー）がこれを読んで判断する
...
3. `<RESULT_FILE>` に `STATUS: BLOCKED - <one-line reason>` を書いて exit
...
- **完了報告ファイル (`<RESULT_FILE>` 等) を repository root に書かないこと。** spawn prompt で指定された repo 外の出力先のみに書く。repository root に result file を作ると `git status` に dirty file として残り、後続 `PDH-review` の diff に巻き込まれて reviewer を混乱させる原因になる (複数 ticket で実発生)。指定先以外への成果物書き出しが必要な場合は上位（team: PM / solo: ユーザー）に依頼する
...
**コードを書く前に、その実装が «確かめていない仮定» を列挙する。**外部 API が返す応答の形、並行して同じ対象を触る他の実行、失敗した途中で残る状態、処理の順序。

- **測れるものは測る。**read-only の呼び出しで確かめられることを推測で書かない
- **測ったより広く書かない。**`404` かつ error code `X` を測ったなら、実装も同じ条件で分岐する。「404 なら何でも」へ広げた分は、測っていない仮定である
- **測れないものは、その前提が崩れたときに何が起きるかまでコードのコメントに書く。**「未確認」とだけ書いても、読んだ人は危険の大きさを判断できない
- ⚠ **実装中に手段を選び直したら、その手段が持ち込む仮定を洗い直す。**ticket を書いた時点に無かった仮定は、note の `Required Probes` では測られていない

## 指摘を直すとき、壊していないことを反例で固定する

finding は「いま間違っている入力」を示す。**同じコードは同時に、正しく扱えていた入力を持つ。**直すと壊れるのはそちら側であり、新しい挙動が正しく動くかだけを確かめても見つからない。

**直す前に、その入力を 1 つ選んで実行し、出力を記録する。**直したあと同じ入力を流し、出力が変わっていたら退行である。

- **記録するのは finding が示した入力ではない。**それは直せば変わるのが当たり前で、退行の検出には使えない
- 選ぶのは、その関数が **拒否していた入力**、**通っていた別の分岐**、**別の実行モード**（並列と逐次、CI とローカル、初回と再実行）
- **報告には「直す前の出力」と「直したあとの同じ入力の出力」を両方貼る。**片方だけでは、変わっていないことを読み手が確かめられない
- 記録できる形で実行できないもの（実環境でしか再現しない等）は、そのことを note に書く。**「変えていないはず」で済ませない**
- 縮小したなら報告に書く。**検出できる範囲を狭める修正は、退行として扱う**（例: 作業ツリー比較を commit 間比較へ変えると、未 commit の変更を検出しなくなる）

例: `[-]` の扱いを変えるなら、変える前に「理由なしの `[-]`」を流して exit 1 を記録しておく。記録していなければ、fail-closed が fail-open に変わったことは誰にも見えない。
...
- credential 不在の場合 → "deferred" として明示 escalate (自己判断で skip しない)（team: PM へ / solo: `<RESULT_FILE>` に記録）。自発的に「stub で十分」と判断しない
...
- **`application-test` と `ticket-local-test` は別物である**。`scripts/test-all.sh`・CI・`test/` に置く `application-test` は、product contract・Architectural Invariants・一般化した regression をカバーする。変更が正しく適用されたかの検証 (例: API リネーム後に旧 URL が 404 を返す、特定の fixture が隠れた) は ticket 固有の一時確認 = `ticket-local-test` であり、テストスイートにコミットしない
- **昇格判定は 1 問**: この挙動を、ticket や一時 fixture の名前を出さずに、継続する product contract (アプリの望ましい状態) として記述できるか。Yes なら `application-test` へコミット、No なら ticket-local のまま close 時に刈る
- **repository が生成物 (bundle 済み worker、compile 済み asset、生成された SDK model 等) を commit しているなら、`application-test` スイートでそれらを再生成して突き合わせる**。再生成した出力が commit 済みファイルと異なるとき fail させ、stale な生成物を reviewer の注意力ではなく決定論で捕まえる
