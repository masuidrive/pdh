...
2. INSTALL.md の「既存プロジェクトのアップデート」の手順に従い、このプロジェクトの PDH を最新版にアップデートする。配置表へ追加されたskillと、directory copy配下に追加された`scripts/checks/*.check`も取り込み、Codex用symlink一覧を現行skill一式へ揃える。**skillと`pdh-*`のagent定義は毎回まるごと上書きする**（差分マージしない）
...
6. 完了報告には、どの項目を適用したか（または該当なしと判断したか）に加えて、**手順 5 の再実行の出力をそのまま貼る。**「適用した」という主張は証拠にならない。
