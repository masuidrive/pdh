...
- [x] **#21 の続き — 3 巡目が起きること自体がおかしい**（ユーザ指摘）。上限は症状の蓋であって原因ではなかった。原因は 2 つ:**(a) 合格条件が無い**（「指摘ゼロ」を目指すと毎巡指摘が返るので終わらない）、**(b) 応じ方が無い**（指摘に AC を足して応え、6 → 10 件になった）。`pdh-verifying` の AC 読み手へ «復元できた AC には何も返さない / AC を増やせという指摘をしない» を、`_flow.md` へ «全 AC が復元できれば合格・AC を足して応えない» を入れた。⚠ **`PDH-AGENTS.md:36` の «修正後は元の finding とその修正差分だけを再 review し、広い探索 review を繰り返し回さない» に、3 巡目の全件パスは既に反していた** — 新しい規則が要る話ではなく、AC 読み手だけがこの既存規則の外で運用されていた
