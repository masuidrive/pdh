...
- **収束する review**: AC trace と severity rubric で重大問題と過剰実装を判別し、修正後は fix delta だけを確認する
...
PDH-implement: 実装
    ↓
PDH-review: AC trace + risk に応じた cross-model review + finding 限定の修正確認
    ↓
...
project-root/
  ...
  tests/
    tickets/
      YYMMDD-hhmmss-slug/
        test-ticket-local.sh ← ticket 固有の一時確認（必要な ticket のみ）
  .claude/
