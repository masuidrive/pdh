...
`pdh-dev` は `SKILL.md` と、そこから参照される `_*.md`（`_flow.md` / `_review.md` / `_execution-team.md` 等）で構成される。**2026-07 以前の README は `SKILL.md` だけをコピーするよう書いていた**ため、手動導入したプロジェクトでは分冊が欠落している可能性がある。これは新規ファイルの追加ではないので `git diff` では検出できない。
