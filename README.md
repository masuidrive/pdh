...
上記に加えて `hooks` ブロックと kill-switch env を追加:
...
意味:
- `CLAUDE_EVENT_DISABLE=1` は default で event 書込を無効化する kill-switch。後で Director が hookbus 消費モードに切り替える時に外す (§tmux Director 参照)
- 4 つの hook は `scripts/hookbus.js` を呼んで `/tmp/claude-events-<socket_hash>/log.ndjson` にイベントを NDJSON で記録する
- 今の時点では **配線だけ完了、log 書込は 0** の dormant 状態
...
4. **新規ファイルの付随設定 (重要)**: 新規ファイルには単独配置だけでは機能しないものがある。下表に該当する追加があれば、対応する章を見て `.claude/settings.json` / `vitest.config.ts` / `scripts/test-all.sh` 等を更新する:
   ...
   |---|---|---|
   | `scripts/hookbus.js` | §2.5.b + §6.5 + §tmux Director | `.claude/settings.json` に 4 hook ブロックと `CLAUDE_EVENT_DISABLE=1`、`vitest.config.ts` に scripts project (includeSource)、`package.json` に `test:scripts`、`scripts/test-all.sh` に `run "test:scripts"` 行 |
...
jq '.hooks.Stop' .claude/settings.json   # 4 hook 配線された JSON が出る
```
...
**前提**: §2 で `scripts/hookbus.js` 配置済、§2.5.b で `.claude/settings.json` に hooks と `CLAUDE_EVENT_DISABLE=1` 配置済 (この時点では配線されているが dormant = log 書込 0)。

**重要な仕様**: `.claude/settings.json` の `env` は **各 Claude Code セッション起動時に読まれて固まる**。mid-session で settings.json を編集しても、走っているセッションの env は変わらない。つまり settings.json から `CLAUDE_EVENT_DISABLE` を外すには **全 Claude セッション一旦終了 → 再起動** が必要。

手順:

1. 全 tmux window で走っている Claude Code セッションを終了 (`/exit` or Ctrl-C)
2. `.claude/settings.json` を編集して **`env.CLAUDE_EVENT_DISABLE` を削除** (or `"0"` に変更)。これで worker 側の hook が log に書くようになる
3. **Director window** だけ特別扱い。自分の Stop / SubagentStop を log に混入させないため、起動コマンドに env を preset:
   ```bash
   CLAUDE_EVENT_ROLE=director claude
   ```
...
- `.claude/settings.json` に `"CLAUDE_EVENT_DISABLE": "1"` を復活 → 全セッション再起動 → log 書込停止
- `/tmp/claude-events-*` の cleanup は `scripts/hookbus.js cleanup --older-than 7` (default) または `rm -rf /tmp/claude-events-*`

詳細は `scripts/hookbus.js` ヘッダコメント (サブコマンド / 制御 env / ログ配置) と `.claude/skills/tmux-director/SKILL.md` の TD-0 節を参照。
